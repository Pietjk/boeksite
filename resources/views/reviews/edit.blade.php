    ...
    <div class="container form-container">
        ...
        <div class="panel is-primary">
            <div class="panel-heading">
                ...
                <span>
                    Pas de review van {{ $review->name }} aan.
                </span>
            </div>
                    ...
                    <div class="columns is-multiline">
                            ...
                            <div class="select is-fullwidth">
                                <select name="book_id">
                                    @foreach ($books as $book)
                                        <option value="{{ $book->id }}" {{ (old('book_id', (string)$review->book_id) === (string)$book->id) ? 'selected' : '' }}>{{ $book->name }}</option>
                                        @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="column is-12">
                            <label for="text" class="label">De naam van de reviewer</label>
                                <input type="text" class="input" name="name" placeholder="Naam" value="{{ old('name', $review->name) }}">
                        </div>
                        <div class="column is-12">
                            <label for="text" class="label">De review</label>
                            <textarea type="text" class="textarea" name="description" placeholder="Text">{{ old('description', $review->description) }}</textarea>
                        </div>
                        <div class="column is-12">
                            <label for="text" class="label">De gegeven score</label>
                            <div class="box range-box">
                                <div class="has-text-centered range-wrap">
                                    <input type="range" class="range" name="score" min="0" max="10" step="0.5" value="{{ old('score', ($review->score !== null) ? $review->score : 0) }}" id="range">
                        ...
                        <div class="column is-12">
                            <label for="text" class="label">De link naar de review</label>
                                <input type="text" class="input" name="link" placeholder="Bijv. https://www.rubenkorfmaker.nl" value="{{ old('link', $review->link) }}">
                        </div>
